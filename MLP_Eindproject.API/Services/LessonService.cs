...
namespace MLP_Eindproject.API.Services
{
    public class LessonService : ILessonService
    {
        ...
        {
            var lessonToBook = await _context.Lessons.Include(x => x.Location).Include(x => x.Teacher).ThenInclude(x => x.Instruments).Include(x => x.Student).FirstOrDefaultAsync(x => x.Id == lessonId);
        ...
        {
            var lessonToCancel = await _context.Lessons.Include(x => x.Location).Include(x => x.Teacher).ThenInclude(x => x.Instruments).Include(x => x.Student).FirstOrDefaultAsync(x => x.Id == lessonId);
        ...
        {
            var lessonToUpdate = await _context.Lessons.Include(x => x.Location).Include(x => x.Teacher).ThenInclude(x => x.Instruments).Include(x => x.Student).FirstOrDefaultAsync(x => x.Id == lessonId);
        ...
        {
            return _context.Lessons.Include(x => x.Location).Include(x => x.Teacher).ThenInclude(x => x.Instruments).Include(x => x.Student).Where(x => x.StudentId == null).ToList();
        }        
        public List<Lesson> GetAllTeacherLessons(int teacherId)
        {
            return _context.Lessons.Include(x => x.Location).Include(x => x.Teacher).ThenInclude(x => x.Instruments).Include(x => x.Student).Where(x => x.TeacherId == teacherId).ToList();
        }
        public List<Lesson> GetAllStudentLessons(int studentId)
        {
            return _context.Lessons.Include(x => x.Location).Include(x => x.Teacher).ThenInclude(x => x.Instruments).Where(x => x.StudentId == studentId).ToList();
        }
        public async Task<Lesson> GetOneLessonById(int id)
        {
            return await _context.Lessons.Include(x => x.Location).Include(x => x.Teacher).ThenInclude(x => x.Instruments).Include(x => x.Student).FirstOrDefaultAsync(x => x.Id == id);
        }
