    ...
    public class CountLessonTests
    {
        ...
        public void CountLessons_Succeeds()
        {
            ...
            using (var scope = TestFixture.ServiceProvider.CreateScope())
            {
                var scopedServices = scope.ServiceProvider;
                var db = scopedServices.GetRequiredService<MLPDbContext>();
                lessonNumbers = new int[] { db.Lessons.Count(), db.Lessons.Where(x => x.StudentId != null).Count(), db.Lessons.Where(x => x.StudentId != null).Where(x => x.Start > DateTime.Now).Count() };
            }
            //Assert
            Assert.Multiple(() =>
            {
                Assert.That(response.IsSuccessStatusCode, "statuscode");
                Assert.That(lessonNumbers[0] == responseArray[0], "Count Total Lessons Correct");
                Assert.That(lessonNumbers[1] == responseArray[1], "Count Booked Lessons Correct");
